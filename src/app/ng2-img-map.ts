 ...
 * Type a title.
 */
  private typeTitle(pixel: number[], title: string): void {
    const context = this.canvas.nativeElement.getContext('2d');
    context.fillText(title, pixel[0]+15, pixel[1]+5);
  }
    ...
    if (this.pixelMarker) {
      this.markers.forEach((marker, index) => {
        if (this.markerActive === index) {
          ...
          this.typeTitle(marker, this.titles[index]);
      ...
      this.pixels.forEach((pixel, index) => {
        if (this.markerActive === index) {
          ...
          this.typeTitle(pixel, this.titles[index]);
